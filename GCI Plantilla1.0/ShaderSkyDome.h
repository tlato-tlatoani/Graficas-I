...
protected:
	void BindAttributeLocation() {
		...
		OpenGL->glBindAttribLocation(m_shaderProgram, 3, "inputTexCoord");
		OpenGL->glBindAttribLocation(m_shaderProgram, 4, "inputTangent");
		OpenGL->glBindAttribLocation(m_shaderProgram, 5, "inputBinormal");
	}
public:
	// Al mismo tiempo que obtenemos los parametros, ejecutamos el constructor de la clase padre
	SkydomeShaderClass(OpenGLClass* OpenGL, HWND hwnd) : Shader(OpenGL, hwnd) {
		this->OpenGL = OpenGL;
		this->hwnd = hwnd;
	}
	// Al mismo tiempo que obtenemos los parametros, ejecutamos el constructor de la clase padre
	// y luego brindamos los archivos para inicializar el shader
	SkydomeShaderClass(OpenGLClass* OpenGL, HWND hwnd, const char* vsFilename, const char* fsFilename) : Shader(OpenGL, hwnd) {
		this->OpenGL = OpenGL;
		this->hwnd = hwnd;
